...
import cleanchicken.lib.packet.PacketCustom.IClientPacketHandler;
import cleanchicken.lib.packet.PacketCustom.IServerPacketHandler;
...
public abstract class ContainerExtended extends Container implements ICrafting
{
    ...
    private void sendNetID(EntityPlayerMP player) {
        if(netID == 0)
            LogManager.getLogger("CleanChickenLib").error("Player added to container with 0 network ID");
        else
    ...
    public PacketCustom getPacket(int type) {
        if(netID == 0)
            LogManager.getLogger("CleanChickenLib").error("Tried to get packet for container with 0 network ID");
        if(type == 1)
