...
import cleanchicken.lib.config.ConfigFile;
import cleanchicken.lib.config.DefaultingConfigFile;
...
public class ASMHelper
{
    ...
    private static ConfigFile loadConfig() {
        try {//weak reference for environments without FML
            ...
            File file = new File(mcDir, "config/CleanChickenLib.cfg");
            if(ObfMapping.obfuscated)
                return new DefaultingConfigFile(file);
            else
                return new ConfigFile(file).setComment("CleanChickenLib development configuration file.");
